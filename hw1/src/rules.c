...
SYMBOL *new_rule(int v) {
    if((v >= FIRST_NONTERMINAL) && (v <= SYMBOL_VALUE_MAX))
    {
        SYMBOL *headNode = new_symbol(v, NULL);
        headNode->rule = headNode;
...
void unref_rule(SYMBOL *rule) {
    ...
    if((rule->refcnt) <= 0)
    {
        fprintf(stderr, "%s\n", "The reference count is already zero and cannot be negative");
        abort();
    }
    (rule->refcnt)--;
}